...
import type { ChatMessage, AdvancedPlayerStats, AltSuspicion, SimilarityMatrix, AlgorithmConfig, SocialInsight, SlipPattern } from "@/lib/types";
import { getPlayerColor } from "@/lib/utils";
import { parseChat, parseMultipleChats } from "@/lib/parser";
import { analyzePlayerAdvanced } from "@/lib/playerAnalysis";
...
import { getGameProfile, GAME_PROFILES } from "@/lib/gameProfiles";
...
export default function GameAnalyzerPage() {
  ...
  const gameId = (params.game as string) || "generic";
        ...
        <div className="bg-bg-secondary rounded-xl border border-border p-4 mb-4 animate-fade-in-up delay-100">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-text-primary">{gameProfile.name}</span>
            </div>
            ...
            <a
              href="/analyzer"
              className="ml-auto px-3 py-1.5 bg-bg-tertiary text-text-secondary rounded-lg hover:bg-bg-tertiary/80 transition-colors text-sm inline-flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <span className="font-semibold text-lg" style={{ color: getPlayerColor(suspicion.player1) }}>
            {suspicion.player1}
          </span>
          <span className="text-text-muted">&#8596;</span>
          <span className="font-semibold text-lg" style={{ color: getPlayerColor(suspicion.player2) }}>
            {suspicion.player2}
          </span>
        ...
        <table className="min-w-full text-xs">
          <thead>
            <tr>
              ...
              {similarityMatrix.players.map(p => (
                <th key={p} className="p-2 font-semibold" style={{ color: getPlayerColor(p) }}>
          ...
          <tbody>
            {similarityMatrix.players.map((p1, i) => (
              <tr key={p1}>
                <td className="p-2 font-semibold" style={{ color: getPlayerColor(p1) }}>
  ...
  return (
    <div className="space-y-6">
      {socialInsights.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-text-primary">Social Relationships ({socialInsights.length})</h3>
          {socialInsights.map((insight, idx) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <span className="font-semibold" style={{ color: getPlayerColor(insight.player1) }}>{insight.player1}</span>
                  <span className="text-text-muted">{insight.insightType === "conflict_detected" ? "&#128683;" : "&#8596;"}</span>
                  <span className="font-semibold" style={{ color: getPlayerColor(insight.player2) }}>{insight.player2}</span>
                  {insight.insightType === "self_talk_suspected" && (
                    <span className="px-2 py-1 bg-error text-white text-xs rounded font-bold">SELF-TALK DETECTED</span>
                  )}
                  {insight.insightType === "conflict_detected" && (
                    <span className="px-2 py-1 bg-warning text-black text-xs rounded font-bold">POSSIBLE CONFLICT</span>
                  )}
                </div>
                <div className={`px-3 py-1 rounded-full text-sm font-semibold ${
                  insight.confidence >= 70 ? "bg-error/20 text-error" : insight.confidence >= 50 ? "bg-warning/20 text-warning" : "bg-info/20 text-info"
                }`}>
              ...
              <div className="mt-3 flex gap-2">
                <button onClick={() => { setSelectedPlayers([insight.player1]); setActiveTab("chat"); }} className="px-3 py-1 bg-bg-tertiary rounded-lg text-text-secondary text-sm hover:bg-bg-tertiary/80">
                  View {insight.player1}
                </button>
                <button onClick={() => { setSelectedPlayers([insight.player2]); setActiveTab("chat"); }} className="px-3 py-1 bg-bg-tertiary rounded-lg text-text-secondary text-sm hover:bg-bg-tertiary/80">
      ...
      {slipPatterns.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-text-primary">Typing Inconsistencies ({slipPatterns.length})</h3>
          ...
          {slipPatterns.map((slip, idx) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <span className="font-semibold text-lg" style={{ color: getPlayerColor(slip.playerName) }}>{slip.playerName}</span>
                  <span className={`px-2 py-1 text-xs rounded font-bold ${
                    slip.suspicionLevel === "high" ? "bg-error text-white" : slip.suspicionLevel === "medium" ? "bg-warning text-black" : "bg-info/20 text-info"
                  }`}>
              ...
              <div className="bg-bg-tertiary rounded-lg p-3">
                <div className="text-xs text-text-muted mb-1">Evidence:</div>
                <ul className="text-sm text-text-secondary list-disc list-inside space-y-1">
                  {slip.evidence.map((e, i) => (<li key={i}>{e}</li>))}
                </ul>
              </div>
              <div className="mt-3">
      ...
      <div className="bg-info/10 border border-info/30 rounded-xl p-4 text-sm">
        ...
        <div className="text-text-secondary space-y-2">
          <p><strong>Self-Talk Detected:</strong> Two accounts talk to each other but have identical writing styles. Real friends write differently - this suggests one person controlling both accounts and talking to themselves.</p>
          <p><strong>Possible Conflict:</strong> Two players are online together but never interact with each other, while actively chatting with others. This may indicate they&apos;re avoiding each other (and are NOT the same person).</p>
          <p><strong>Typing Inconsistencies:</strong> A player&apos;s writing style changes during the session - they might be &quot;slipping&quot; back to their natural style after trying to type differently.</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="bg-bg-secondary rounded-xl border border-border p-4">
        <h3 className="text-lg font-semibold text-text-primary mb-4">Forensics Lab - Direct Comparison</h3>
      ...
      {getCompareStats && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {getCompareStats.map((stats, idx) => (
            <ForensicsPlayerCard key={stats.name} stats={stats} otherStats={getCompareStats[1 - idx]} />
          ))}
        </div>
      )}
      ...
      <div className="space-y-4 text-sm">
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-bg-tertiary p-2 rounded"><span className="text-text-muted">Yule&apos;s K:</span> <span className="text-text-primary font-mono">{stats.yulesK}</span></div>
            <div className="bg-bg-tertiary p-2 rounded"><span className="text-text-muted">Vocab:</span> <span className="text-text-primary font-mono">{(stats.vocabularyRichness * 100).toFixed(1)}%</span></div>
            <div className="bg-bg-tertiary p-2 rounded"><span className="text-text-muted">Hapax:</span> <span className="text-text-primary font-mono">{(stats.hapaxRatio * 100).toFixed(1)}%</span></div>
            <div className="bg-bg-tertiary p-2 rounded"><span className="text-text-muted">Wrd len:</span> <span className="text-text-primary font-mono">{stats.avgWordLength}</span></div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {stats.typoPatterns.length > 0 ? stats.typoPatterns.map(t => (
              <span key={t} className={`px-2 py-1 rounded text-xs ${otherStats.typoPatterns.includes(t) ? "bg-error text-white font-bold" : "bg-bg-tertiary text-text-secondary"}`}>{t}</span>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {stats.letterSubstitutions.size > 0 ? [...stats.letterSubstitutions.keys()].map(s => (
              <span key={s} className={`px-2 py-1 rounded text-xs ${otherStats.letterSubstitutions.has(s) ? "bg-warning text-black font-bold" : "bg-bg-tertiary text-text-secondary"}`}>{s}</span>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {stats.commonPhrases.slice(0, 5).map(p => (
              <span key={p} className={`px-2 py-1 rounded text-xs ${otherStats.commonPhrases.includes(p) ? "bg-accent text-white font-bold" : "bg-bg-tertiary text-text-secondary"}`}>&quot;{p}&quot;</span>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {stats.punctuationStyle.spaceBefore && (
              <span className={`px-2 py-1 rounded text-xs ${otherStats.punctuationStyle.spaceBefore ? "bg-success text-white" : "bg-bg-tertiary text-text-secondary"}`}>space before ?!</span>
            )}
            {stats.punctuationStyle.doublePunctuation && (
              <span className={`px-2 py-1 rounded text-xs ${otherStats.punctuationStyle.doublePunctuation ? "bg-success text-white" : "bg-bg-tertiary text-text-secondary"}`}>!! / ??</span>
            )}
            {stats.punctuationStyle.ellipsisStyle !== "none" && (
              <span className={`px-2 py-1 rounded text-xs ${otherStats.punctuationStyle.ellipsisStyle === stats.punctuationStyle.ellipsisStyle ? "bg-success text-white" : "bg-bg-tertiary text-text-secondary"}`}>{stats.punctuationStyle.ellipsisStyle}</span>
            )}
          </div>
        </div>
